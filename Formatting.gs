function formatWorkbook_() {
  ...
  ['Promos','BTO','NonPromos','Monthly Metrics'].forEach(n=>{const s=mbSheet_(n);const cols=s.getLastColumn();s.getRange(1,1,1,cols).setBackground(navy).setFontColor('white').setFontWeight('bold').setWrap(true);s.setRowHeight(1,42);s.getRange(2,n==='BTO'?1:2,MB.ROWS-1,1).setNumberFormat('dd/mm/yyyy');if(n!=='BTO'&&n!=='Monthly Metrics')s.getRange(2,16,MB.ROWS-1,1).setNumberFormat('MMM yyyy');});
  ...
  const formats={Promos:{money:[6,10,11,12,13],pct:[9,14]},BTO:{money:[5,9,11,12,15,16],pct:[10,14,17]},NonPromos:{money:[6,10,12,13,14],pct:[11,15]}};
  ...
  [['Promos',[1,9,10,13,14,15,16]],['BTO',[9,10,16,17]],['NonPromos',[1,10,11,14,15,16]]].forEach(x=>protectFormulaColumns_(mbSheet_(x[0]),x[1]));
