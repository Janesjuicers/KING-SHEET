function runAudit() {
  ...
  const tests=[
    ['Promo totals versus Monthly Metrics','=IF(ABS(SUM(Promos!I6:I)+SUM(Promos!J6:J)-SUM(\'Monthly Metrics\'!E2:E))<=0.01,"PASS","FAIL")','=SUM(Promos!I6:I)+SUM(Promos!J6:J)-SUM(\'Monthly Metrics\'!E2:E)','$0.01','Cash Change plus Bonus Change versus monthly aggregate'],
    ['Promo totals versus Master PnL','=IF(ABS(SUM(Promos!I6:I)+SUM(Promos!J6:J)-\'Master PnL\'!B6)<=0.01,"PASS","FAIL")','=SUM(Promos!I6:I)+SUM(Promos!J6:J)-\'Master PnL\'!B6','$0.01','Cash Change plus Bonus Change versus dashboard'],
    ['Promo EV versus Monthly Metrics','=IF(ABS(SUM(Promos!H6:H)-SUM(\'Monthly Metrics\'!C2:C))<=0.01,"PASS","FAIL")','=SUM(Promos!H6:H)-SUM(\'Monthly Metrics\'!C2:C)','$0.01',''],
    ...
    ['Missing Promo Type Settings rates','=IF(SUM(ARRAYFORMULA(N(Promos!C6:C<>"")*N(ISNA(Promos!H6:H))))=0,"PASS","FAIL")','=SUM(ARRAYFORMULA(N(Promos!C6:C<>"")*N(ISNA(Promos!H6:H))))','0','EV Taken performs the Settings lookup directly'],
    ...
    ['Duplicate Entry IDs','=IF(SUM(ARRAYFORMULA(N(COUNTIF({BTO!A2:A;NonPromos!A2:A},{BTO!A2:A;NonPromos!A2:A})>1)*N({BTO!A2:A;NonPromos!A2:A}<>"")))=0,"PASS","FAIL")','','0','Promos intentionally have no Entry ID'],
    ['Invalid dates','=IF(SUM(ARRAYFORMULA(N({Promos!A6:A;BTO!B2:B;NonPromos!B2:B}<>"")*N(NOT(ISNUMBER({Promos!A6:A;BTO!B2:B;NonPromos!B2:B}))))))=0,"PASS","FAIL")','','0',''],
    ['$100 SGM3 Leg at configured rate','=IF(ROUND(100*VLOOKUP("SGM3 Leg",Settings!A:B,2,FALSE),2)=ROUND(100*Settings!B2,2),"PASS","FAIL")','=ROUND(100*VLOOKUP("SGM3 Leg",Settings!A:B,2,FALSE),2)','$25.00 initially; $30.00 after changing the rate to 30%','Uses the same direct lookup as EV Taken'],
